...
class TaskCRUDTest(TestCase):
    ...
    def test_delete_own_task(self):
        task = Task.objects.create(name='Моя задача',
                                   author=self.user1, status=self.status)
        response = self.client.post(reverse('task_delete', args=[task.pk]))
        self.assertEqual(Task.objects.count(), 0)
        self.assertRedirects(response, reverse('tasks_list'))

    def test_cannot_delete_other_user_task(self):
        task = Task.objects.create(name='Чужая задача', author=self.user2,
                                   status=self.status)
        response = self.client.post(reverse('task_delete', args=[task.pk]))
        self.assertEqual(Task.objects.count(), 1)
        self.assertRedirects(response, reverse('tasks_list'))
...
class TaskFilterTest(TestCase):
    ...
    def test_filter_by_status(self):
        response = self.client.get(reverse('tasks_list'), {'status': self.status_new.id})
    ...
    def test_filter_by_label(self):
        response = self.client.get(reverse('tasks_list'), {'labels': self.label_bug.id})
        self.assertContains(response, 'Задача 1')
        self.assertNotContains(response, 'Задача 2')
